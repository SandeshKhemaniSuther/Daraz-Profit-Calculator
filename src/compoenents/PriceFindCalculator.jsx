...
function PriceCalculator({ onResultChange }) {
  ...
  const handleInputChange = (setter, val) => {
    setter(val);
    setResults(null);
    onResultChange(false); // Reset dynamic outer scroll when user alters input
  };

  const handleCalculate = (e) => {
    e.preventDefault();
    const payoutAmount = parseFloat(targetPayout);
    if (!payoutAmount || payoutAmount <= 0 || !selectedCategory) {
      alert("Please specify a valid target payout amount.");
      return;
    }

    const commissionFactor = commissionRate / 100;
    const paymentFactor = PAYMENT_FEE_RATE / 100;
    const serviceGstFactor = ((commissionRate + PAYMENT_FEE_RATE) * provincialVatRate) / 10000;
    const fbrFactor = fbrRate / 100;

    const netPayoutRatio = 1 - (commissionFactor + paymentFactor + serviceGstFactor + fbrFactor);

    if (netPayoutRatio <= 0) {
      alert("Calculation conflict: Taxes exceed structure values.");
      return;
    }

    const requiredProductPrice = payoutAmount / netPayoutRatio;
    const darazCommission = (requiredProductPrice * commissionRate) / 100;
    const paymentFee = (requiredProductPrice * PAYMENT_FEE_RATE) / 100;
    const darazServiceTax = ((darazCommission + paymentFee) * provincialVatRate) / 100;
    const fbrTax = (requiredProductPrice * fbrRate) / 100; 
    ...
    setResults({
      ...
      requiredPrice: requiredProductPrice.toFixed(2),
    });
    
    onResultChange(true); // Outer dynamic scroll active command
  };
    ...
    <div className={`grid grid-cols-1 gap-4 items-start h-auto transition-all duration-500 ${results ? 'md:grid-cols-2' : 'grid-cols-1'}`}>
      ...
      {results && (
        <div className="space-y-3 flex flex-col justify-start h-auto border-t border-slate-800/60 md:border-t-0 md:border-l pt-4 md:pt-0 md:pl-4 overflow-visible">
          <div>
            <h3 className="text-[9px] font-bold tracking-widest text-slate-500 uppercase font-mono mb-1.5">Estimated Deductions</h3>
          ...
          <div className="relative mt-0.5 h-auto">
            ...
            <div className="relative bg-[#040612] border border-cyan-500/20 rounded-lg p-2.5 flex justify-between items-center gap-4 h-auto">
              <div className="min-w-0">
                <span className="block text-[8px] font-bold tracking-wider text-slate-500 uppercase font-mono">Required Retail Price</span>
                <span className="text-base sm:text-lg md:text-xl font-black text-transparent bg-gradient-to-r from-cyan-400 to-fuchsia-400 bg-clip-text block font-mono tracking-tight truncate">
                  Rs {results.requiredPrice}
                </span>
              </div>
              <div className="px-1.5 py-0.5 bg-cyan-950/80 border border-cyan-800 text-[8px] font-mono text-cyan-400 rounded font-bold uppercase tracking-wider shrink-0">
                Target Price
              </div>
            </div>
          </div>
